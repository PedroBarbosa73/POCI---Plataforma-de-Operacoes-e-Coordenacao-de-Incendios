...
const { incidents, units, alerts, radioMessages, fireStations, closures, initialZones } =
  await import(pathToFileURL(mockDataPath).href)
...
async function run() {
  ...
  if (initialZones) {
    const zoneRows = Object.entries(initialZones).flatMap(([incidentId, zones]) =>
      (zones ?? []).map(z => ({
        // omit id — zones use UUID PKs generated by Supabase
...
async function upsert(table, rows) {
  ...
  // unit_states uses unit_id as PK; zones/closures/alerts use uuid PKs (server-generated)
  const conflictCol = table === 'unit_states' ? 'unit_id' : 'id'
